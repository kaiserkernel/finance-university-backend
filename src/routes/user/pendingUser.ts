...
router.get('/:email', async (req: any, res: Response) => {
    ...
    const query = queryByRoleWritter(role, user?.college ?? undefined)
    console.log(query)
    User.find(query, '_id firstName lastName email college enrollment role allowed rejected').then(result => {
        if (isEmpty(result)) {
